...
const confirmPickup = async (req, res) => {
  try {
    ...
    if (listing.status !== 'claimed')
      return res
        .status(400)
        .json({ message: 'Listing not in claimed state' });
    ...
    if (listing.claimedBy) {
      ...
      if (vol.mealsRescued >= 5 && !badges.has('First Five'))
        badges.add('First Five');

      if (vol.mealsRescued >= 50 && !badges.has('Hunger Hero'))
        badges.add('Hunger Hero');

      if (vol.mealsRescued >= 200 && !badges.has('Food Guardian'))
        badges.add('Food Guardian');
